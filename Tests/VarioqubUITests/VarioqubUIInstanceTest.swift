...
@Suite("VarioqubUI")
struct VarioqubUIInstanceTest {
    
    private let varioqubMock: VarioqubInstanceMock
    private let varioqubUI: VarioqubUIInstance
    ...
    init() {
        ...
        varioqubUI = VarioqubUIInstance(instance: varioqubMock)
    ...
    func testApplyWidget() async throws {
        try await confirmation(expectedCount: 1) { confirmation in
            ...
            try await varioqubUI.applyWidget(widgetName: widgetName, widgetApplier: varioqubWidgetMock)
        }
    }
    ...
    func testApplyWidgetWithOverrideParams() async throws {
        ...
        try await confirmation(expectedCount: 1) { confirmation in
            ...
            try await varioqubUI.applyWidget(widgetName: widgetName, widgetApplier: varioqubWidgetMock, extraParams: overrideParams)
        }
    }
    ...
    func testApplyWidgetWithURL() async throws {
        ...
        try await confirmation(expectedCount: 1) { confirmation in
            ...
            try await varioqubUI.applyWidget(
    ...
    func testApplyWidgetWithEmptyValue() async throws {
        await #expect(throws: VarioqubError.self) {
            try await varioqubUI.applyWidget(widgetName: widgetNameEmpty, widgetApplier: varioqubWidgetMock)
        }
    }
